# Bilibili Agent：B站本地 RAG 问答系统

一个本地运行的B站智能问答系统。用户输入 B 站关键词或直接粘贴 BV 号/视频链接，系统会搜索公开视频、下载音频、语音转文字、构建本地 RAG 索引，并使用本地 Qwen 模型基于视频上下文进行问答。

项目适合课程复习、视频知识点检索、视频内容总结和本地隐私保护场景。
